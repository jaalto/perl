...
recognized; barewords are considered filenames.
    ...
    open my $fh, "<", "foo"  or  die "Opening 'foo' failed: $!";
...
cantankerous), you might need something like this:
    ...
    for (;;) {
        for ($curpos = tell(FILE);		# One
	     $_ = <FILE>;			# Two
	     $curpos = tell(FILE)) {		# Three
...
whether your Perl supports quads via L<Config>:
    ...
    use Config;
    if ($Config{use64bitint} eq "define"  or  $Config{longsize} >= 8) {
        print "Nice quads!\n";
    }
...
It can also be that long doubles and doubles are the same thing:
    ...
    use Config;
    ($Config{doublesize} == $Config{longdblsize}) and
